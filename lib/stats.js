  ...
  log(actorName, requestID, action, error=null, message) {
    ...
    switch (action) {
      ...
      case 'process':
        ...
        break;
      case 'error':
        console.log('ERROR!', this.stats.actors[actor][request], error);
        statEntry.end = this.getTimeStamp();
        statEntry.elapsed = statEntry.end - statEntry.start;
        statEntry.success = false;
        if (error !== null) {
  ...
  getStats() {
    ...
    Object.keys(this.stats.actors).forEach((actor) => {
      ...
      Object.keys(this.stats.actors[actor]).forEach((request) => {
        ...
        // TK; This is what I added to track messages with no responses
        if (this.stats.actors[actor][request].success === false) {
          console.log('Request with no response', request, this.stats.actors[actor][request]);
        }
